...
static void uncompressThread(string binFile)
{
	AssetsManager::uncompress(binFile);
}

static int lua_cocos2dx_uncompress(lua_State* L)
{
	if (nullptr == L)
		return 0;
	...
	if (1 == argc)
	{
		...
		if (!tolua_isstring(L, 1, 0, &tolua_err))
			goto tolua_lerror;
#endif
		std::string binFile = tolua_tostring(L, 1, "");
		
		std::thread first(uncompressThread, binFile);
		first.detach();

		return 0;
	}
	CCLOG("'uncompress' function wrong number of arguments: %d, was expecting %d\n", argc, 1);
	return 0;
...
int register_assetsmanager_test_sample(lua_State* L)
{
	...
	tolua_function(L, "uncompress", lua_cocos2dx_uncompress);
